...
export default function LaMulanaDescription({
	...
	const metaRows = [
		{ label: 'Title',         value: getLocalizedTitle(title) },
		{
			label: 'Circle',
			value: (
				<Link href={circle.link} style={{ color: '#e8b840', textDecoration: 'none' }}>
					{circle.name}
				</Link>
			),
		},
				...
				<ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
					{storeLinks.map(({ key, name, link }) => (
						<li key={key} style={{ marginBottom: '0.2rem' }}>
							<span style={{ color: '#c8a060', opacity: 0.7, marginRight: '0.3em' }}>&gt;</span>
							{link ? (
	...
	return (
		<section className={lmFontVars} style={{
			background: 'radial-gradient(ellipse at 60% 40%, #5a3e1b 0%, #3a2810 45%, #1e1508 100%)',
			...
			<div className="lm-desc-grid" style={{
				maxWidth: 960,
				margin: '0 auto',
				display: 'grid',
					...
					<Link href={cover} target="_blank" rel="noopener noreferrer">
						<Image
							...
							style={{
								...
								height: 'auto',
								display: 'block',
								boxShadow: '0 0 0 2px #7a5828, 0 0 0 4px #3a2810, 0 8px 32px rgba(0,0,0,0.7)',
							}}
						/>
					</Link>
					{/* Worn-edge vignette */}
				...
				<div style={{ display: 'flex', flexDirection: 'column', paddingTop: '0.25rem' }}>
					{metaRows.map(({ label, value }, i) => (
						<div key={label} style={{
							...
							borderTop: i === 0 ? '1px solid rgba(160,120,64,0.25)' : undefined,
							display: 'flex',
