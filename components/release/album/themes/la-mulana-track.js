...
export default function LaMulanaTrack({ track, openDialog }) {
	...
	const title     = typeof trackData.title === 'object' ? trackData.title[locale] : trackData.title;
	const artist    = typeof trackData.artist === 'object' ? trackData.artist[locale] : trackData.artist;
	const { credits, aboutText, links } = parseDescription(trackData.description);

	const hasCredits = credits.length > 0 || links.length > 0 || !!aboutText;

	const toggle = () => { if (hasCredits) setIsOpen(o => !o); };
			...
			{hasCredits && (
				<div style={{
					...
					<div style={{ overflow: 'hidden' }}>
						<div style={{
							padding: `0.28rem 1.65rem 1.38rem calc(1.65rem + 2.2rem + 1.1rem)`,
							...
							{links.length > 0 && (
								<div style={{
									display: 'flex',
									gap: '0.83rem',
									marginTop: credits.length > 0 ? '0.55rem' : 0,
									marginBottom: '0.44rem',
									lineHeight: 1.5,
								}}>
									<span style={{
									...
									<span style={{ fontFamily: STM, fontSize: '0.82rem' }}>
										{links.map((link, i) => (
											<a
												key={i}
												href={link}
												...
												style={{ color: LM.amber, textDecoration: 'none', display: 'block' }}
											>
												{link.replace(/^https?:\/\//, '')}
											</a>
										))}
							...
							{aboutText && (
								<button
									...
									onClick={(e) => {
										e.stopPropagation();
										openDialog({ title, artist, aboutText });
									}}
								>
									About this track <span style={{ fontSize: '0.75rem', marginLeft: 4 }}>→</span>
								</button>
							)}
						</div>
					</div>
