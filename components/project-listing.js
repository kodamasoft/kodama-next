...
export default function ProjectListing({ project, category }) {
	...
	useEffect(() => {
		...
		if (project.deadline)
			setDeadline(
				t(
					new Date(project.deadline).toLocaleDateString(lang, {
						year: 'numeric',
						month: 'short',
						day: 'numeric',
					})
	...
	function statusBadgeStyles(status) {
		let styles =
			'inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold text-white rounded-full text-base';
	...
	return (
		<Link
			...
			key={project.slug}
			className={
				'flex flex-col text-center no-underline not-prose md:max-w-2xl mx-auto content-center rounded-xl my-5 transition py-5 px-10 relative hover:-translate-y-1 '
			}
		...
		>
			...
			<p>
				<span className={statusBadgeStyles(project.status)}>
					{status}
				</span>{' '}
			...
			{deadline && (
				<p>
					{t('projects:common.deadline')}: {deadline}
				</p>
			)}
		</Link>
	);
